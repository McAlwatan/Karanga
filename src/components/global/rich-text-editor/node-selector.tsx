...
const NodeSelector = ({ open, onOpenChange }: Props) => {
    ...
    <Popover modal={true} open={open} onOpenChange={onOpenChange}>
      ...
      >
        <Button type="button" variant="ghost" className="gap-2 ">
          <span className="whitesoace-nowrap text-sm">{activeItem.name}</span>
      ...
      <PopoverContent sideOffset={5} align="start" className="w-48 p-1">
        {items.map((item, index) => (
          <EditorBubbleItem
            ...
            className="flex curosr-pointer items-center justify-between rounded-sm px-2 py-1 text-sm hover:bg-accent"
          >
